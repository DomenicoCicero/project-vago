...
let selectedImage = innerImages[0];
...
const updateThumbnailGallery = () => {
  const thumbnailGallery = document.getElementById("thumbnailGallery");
  thumbnailGallery.innerHTML = "";

  innerImages.forEach((image, index) => {
    const thumbnailDiv = document.createElement("div");
    thumbnailDiv.className = `thumbnail ${selectedImage === image ? "active" : ""}`;
    thumbnailDiv.onclick = () => handleImageClick(image);
...
const generateCarousel = () => {
  ...
  for (let i = 0; i < yachts.length; i++) {
    ...
    col.classList.add("col-4", "d-flex", "align-items-center");

    const link = document.createElement("a");
    link.classList.add();
    link.href = `/product-details.html?id=${yachts[i].id}`;
    ...
    name.innerText = yachts[i].name;
    name.classList.add("position-absolute", "text-center", "w-100", "carousel-name-item");

    const img = document.createElement("img");
    img.src = yachts[i].immagini[0];
    img.classList.add("d-block", "w-100");

    collectionCard.appendChild(img);
    collectionCard.appendChild(name);
    link.appendChild(collectionCard);
